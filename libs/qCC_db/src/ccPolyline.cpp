...
ccPolyline::ccPolyline(GenericIndexedCloudPersist *associatedCloud, unsigned uniqueID /*=ccUniqueIDGenerator::InvalidUniqueID*/)
	: Polyline(associatedCloud), ccShiftedObject("Polyline", uniqueID)
{
	set2DMode(false);
	setForeground(true);
	setVisible(true);
...
ccPolyline::ccPolyline(const ccPolyline &poly)
	: Polyline(nullptr), ccShiftedObject(poly)
{
...
void ccPolyline::setDrawLine(bool state)
{
	m_drawLine = state;
}
...
void ccPolyline::drawMeOnly(CC_DRAW_CONTEXT &context)
{
	...
	if (m_drawLine)
	{
		...
		for (unsigned i = 0; i < vertCount; i += 2)
		{
			ccGL::Vertex3v(glFunc, getPoint(i)->u);
			ccGL::Vertex3v(glFunc, getPoint(i + 1)->u);
		}
		glFunc->glEnd();
		if (m_width != 0)
		{
...
bool ccPolyline::toFile_MeOnly(QFile &out, short dataVersion) const
{
	...
	// Draw line mode (dataVersion>=40)
	outStream << m_drawLine;
...
bool ccPolyline::createNewPolylinesFromSelection(std::vector<ccPolyline *> &output)
{
	...
	bool success = true;
	{
		...
		for (unsigned i = 0; i < maxIndex; ++i)
		{
			...
			bool kept = false;
			if (verticesVisibility.at(pointIndex) == CCCoreLib::POINT_VISIBLE && verticesVisibility.at(nextPointIndex) == CCCoreLib::POINT_VISIBLE) // segment should be kept
			{
				kept = true;
